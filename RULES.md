...
- **Brand logo:** always the PNG asset (`config.assets.logo`). On public pages it links to the marketing home (`/`); inside the `(dashboard)` shell it links to the app's own home, a signed-in user clicking the logo should never get bounced out to the marketing page — human decision 2026-08-31.
- **App home is `config.app.homePath` — human decision 2026-09-04.** It has moved twice (`/vault` -> `/runs` 2026-09-01, `/runs` -> `/dashboard` 2026-09-04), so it is a config value and NOT a literal: the logo, the login redirect and its `callbackURL`, the PWA `start_url`, the landing page's CTA, and every "back to the app" affordance read it. A link to the runs LIST (the sidebar's Runs item, the pager, a run's back button) is not home and stays `/runs`.
